...
async function getCollection(): Promise<{ collection: any; name: string } | null> {
  ...
  const candidates = ['candidates', 'candidats', 'leads', 'contacts', 'persons', 'people', 'profiles', 'profils', 'users'];
  for (const n of candidates) {
    if (names.includes(n)) return { collection: db.collection(n), name: n };
  }
  // Fallback: pick the largest collection
...
router.get('/stats', async (req: Request, res: Response) => {
  try {
    ...
    const possibleNames = [
      'candidates', 'candidats', 'leads', 'contacts',
      'persons', 'people', 'profiles', 'profils', 'users', 'candidature',
    ];
...
router.get('/inspect', async (req: Request, res: Response) => {
  try {
    ...
    const possibleNames = [
      'candidates', 'candidats', 'leads', 'contacts',
      'persons', 'people', 'profiles', 'profils', 'users',
    ];
